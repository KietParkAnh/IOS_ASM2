...
struct NormalGame: View {
    public var fiveColumnGrid = [GridItem(.flexible()),
                                  GridItem(.flexible()),
                                  GridItem(.flexible()),
                                  GridItem(.flexible())]

    @State private var cards = createMediumCardList().shuffled()
    @State private var MatchedCards = [Card]()
    @State private var UserChoices = [Card]()
    @State private var gameTimeRemaining = 120 // 2 minutes
...
struct NormalGame_Previews: PreviewProvider {
    @State static var selectedLevel: String? = "Normal" // Provide a default level for preview

    static var previews: some View {
        NormalGame(score: .constant(5), selectedLevel: $selectedLevel)
